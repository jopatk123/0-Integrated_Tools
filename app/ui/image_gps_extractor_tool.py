...
class ImageGPSExtractorTool:
    ...
    def scan_folder(self, folder):
        ...
        for root, _, files in os.walk(folder):
            for file in files:
                if file.lower().endswith(image_extensions):
                    count += 1
                    self.status_var.set(f"已扫描 {count} 张图片...")
        ...
        self.parent_frame.after(0, self.update_status, f"扫描完成，共找到 {count} 张图片")
    ...
    def import_excel(self):
        ...
        try:
            ...
            self.status_var.set(f"已导入 {count} 张图片信息")
    ...
    def rename_images(self):
        ...
        self.status_var.set(f"已重命名 {self.renamed_count} 张图片")
    ...
    def rename_folders(self):
        ...
        self.status_var.set(f"已重命名 {renamed_count} 个文件夹")